"""Scrape VTuber rankings and agency directories from userlocal ranking site."""
...
try:
    for i in range(40):
        ...
        table = driver.find_element(By.CSS_SELECTOR, "table.table-ranking-yt")
        rows = table.find_elements(By.TAG_NAME, "tr")

        for row in rows:
            info_col = row.find_element(By.CSS_SELECTOR, "td.col-name")
            stat_col = row.find_element(By.CSS_SELECTOR, "td.vertical")
            ...
            rank = rank_elements[0].text.split("位")[0] if rank_elements else None
            ...
            if box_office_elements:
                office_elements = box_office_elements[0].find_elements(By.CSS_SELECTOR, 'a[href^="/office"]')
                office_name = office_elements[0].text if office_elements else None
            else:
                office_name = None
            ...
            if office_name:
                if office_name not in seen_offices:
                    office_list.append({"office_id": office_id, "office_name": office_name})
                    seen_offices.add(office_name)
                    current_office_id = office_id
                    office_id += 1
                else:
                    current_office_id = next(
                        o["office_id"] for o in office_list if o["office_name"] == office_name
                    )
            else:
            ...
            print(f"Noted #{rank} entry (id={vtuber_id}): {vtuber_name}")
            vtuber_id += 1
